...
public class CommentController {
	...
	@ApiOperation(value = "qnano에 해당하는 도서평 목록을 반환한다.", response = List.class)
	@GetMapping("{qnano}")
	public ResponseEntity<List<CommentDto>> listComment(@PathVariable("qnano") int qnano) throws Exception {
	...
	@ApiOperation(value = "글번호가 no에 해당하는 도서평을 수정한다. 그리고 DB수정 성공여부에 따라 'success' 또는 'fail' 문자열을 반환한다.", response = String.class)
	@PutMapping
	...
	@ApiOperation(value = "글번호가 no에 해당하는 도서평을 삭제한다. 그리고 DB삭제 성공여부에 따라 'success' 또는 'fail' 문자열을 반환한다.", response = String.class)
	@DeleteMapping("{no}")
	public ResponseEntity<String> deleteBook(@PathVariable("no") int no) throws Exception {
		logger.debug("deleteBook - 호출");
		if(commentService.delete(no)) {
			return new ResponseEntity<String>(SUCCESS, HttpStatus.OK);
		}
		return new ResponseEntity<String>(FAIL, HttpStatus.NO_CONTENT);
	}
